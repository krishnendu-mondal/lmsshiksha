...
<body>
    ...
    if (isset($_POST["save_password"])) {
        ...
        if(mysqli_num_rows($res)>0){
            if ($current_pass != $row['password']) {
                ...
                $sql = "UPDATE `student` SET `password` = '{$new_pass}' WHERE `email` = '{$student_email}'";
    ...
    if (isset($_POST["save_email"])) {
        ...
        $sql = "SELECT `name` FROM `student` WHERE `email` = '{$new_email}'";
        $res = mysqli_query($conn, $sql) or die("Query failed!");
        if (mysqli_num_rows($res) > 0) {
            mysqli_close($conn);
            echo ("<script>showEmailErrorToast();</script>");
        } else {
            $sql = "UPDATE `student` SET `email` = '{$new_email}' WHERE `roll` = '{$student_roll}'";
            mysqli_query($conn, $sql) or die("Query failed!");
            mysqli_close($conn);
    ...
    if (isset($_POST["delete"])) {
        require_once("DB.php");
        $sql = "DELETE FROM `student` WHERE `roll` = '$student_roll'";
        mysqli_query($conn, $sql) or die("Query failed!");
        session_destroy();
