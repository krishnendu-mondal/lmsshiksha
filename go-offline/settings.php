...
<body>
    ...
    <div class="container">
                ...
                <ul>
                    <li>
                        <p onclick="window.location.href='dashboard.php'"><i
                                class="ri ri-shield-user-fill"></i><span class="show-user-id-sidebar">
                                <?php echo $admin_user_id?>
                            </span></p>
                    </li>
                    <li>
                    ...
                    <li>
                        <p onclick="window.location.href='edit-user.php'"><i
                                class="fa fa-solid fa-user-pen fa-xs"></i><span>Edit user info</span></p>
                    </li>
                    <li>
                    ...
                    <li>
                        <p id="active"><i class="ri ri-settings-4-fill"></i><span>Settings</span></p>
                    </li>
                </ul>
                <div class="logout">
            ...
            <div class="panel profile-panel">
                ...
                <div class="infos">
                    <div class="flex">
                        ...
                        <abbr title="Edit" onclick="show_edit_name_screen()"><button><i
                                    class="ri-edit-2-line"></i></button></abbr>
                    </div>
                    ...
                    <div class="flex">
                        ...
                        <abbr title="Edit"><button onclick="show_edit_user_id_screen()"><i
                                    class="ri-edit-2-line"></i></button></abbr>
                    </div>
        ...
        <div class="popup popup3">
            <form action="settings.php" method="post">
                ...
                <div class="grid eml">
                    <label>Name</label>
                    <input type="text" name="new_name" placeholder="Name" required autocomplete="off">
                </div>
                <div class="grid">
                    <div class="flex">
                        <button class="btn-save" type="submit" name="save_name" value="save"><i
                                class="ri-checkbox-circle-line"></i> Save</button>
                        <button class="btn-cancel" type="reset" onclick="hide_edit_name_screen()">Cancel</button>
                    </div>
                </div>
        ...
        <div class="popup popup4">
            <form action="settings.php" method="post">
                <span>Enter new user id</span>
                <div class="grid eml">
                    <label>User ID</label>
                    <input type="text" name="new_user_id" placeholder="User ID" required autocomplete="off">
                </div>
                <div class="grid">
                    <div class="flex">
                        <button class="btn-save" type="submit" name="save_user_id" value="save"><i
                                class="ri-checkbox-circle-line"></i> Save</button>
                        <button class="btn-cancel" type="reset" onclick="hide_edit_user_id_screen()">Cancel</button>
                    </div>
                </div>
            ...
            <form action="settings.php" method="post">
                ...
                <div class="pos-sent">
                    <div class="grid eml">
                        ...
                        <input type="password" id="pass1" name="curr_password" placeholder="Old password" required autocomplete="off">
                        <i class="ri-eye-off-line pass-show-hide1 eye-closed"></i>
                    </div>
                    <div class="grid eml">
                        <label>New password</label>
                        <input type="password" id="pass2" name="new_password" placeholder="New password" required autocomplete="off">
    ...
    <?php
        if(isset($_POST["save_password"])){
            ...
            $sql = "SELECT `password` FROM `admin` WHERE `user_id` = '{$admin_user_id}'";
            ...
            if(mysqli_num_rows($res)>0){
                if($current_pass != $row['password']){
                    ...
                    $sql = "UPDATE `admin` SET `password` = '{$new_pass}' WHERE `user_id` = '{$admin_user_id}'";
        ...
        if(isset($_POST["save_name"])){
            ...
            $sql = "UPDATE `admin` SET `name` = '{$new_name}' WHERE `user_id` = '{$admin_user_id}'";
            mysqli_query($conn, $sql) or die("Query failed!");
            mysqli_close($conn);
            $_SESSION["admin_name"] = $new_name;
            echo "<script>display_updated_name("."'".$new_name."'".")</script>";
            echo("<script>showSuccessToast();</script>");

        }

        if(isset($_POST["save_user_id"])){
            require_once("DB.php");
            $new_user_id = mysqli_real_escape_string($conn, $_POST["new_user_id"]);
            $sql = "SELECT `name` FROM `admin` WHERE `user_id` = '{$new_user_id}'";
            $res = mysqli_query($conn, $sql) or die("Query failed!");
            if(mysqli_num_rows($res) > 0){
                mysqli_close($conn);
                echo("<script>showadmin_user_idErrorToast();</script>");
            }else{
                $sql = "UPDATE `admin` SET `user_id` = '{$new_user_id}' WHERE `user_id` = '{$admin_user_id}'";
                mysqli_query($conn, $sql) or die("Query failed!");
                mysqli_close($conn);
                $_SESSION["admin_user_id"] = $new_user_id;
                echo "<script>display_updated_user_id("."'".$new_user_id."'".")</script>";
                echo("<script>showSuccessToast();</script>");
            }
        }
        ...
        if(isset($_POST["save_email"])){
            ...
            $sql = "SELECT `name` FROM `admin` WHERE `email` = '{$new_email}'";
            $res = mysqli_query($conn, $sql) or die("Query failed!");
            if(mysqli_num_rows($res) > 0){
                mysqli_close($conn);
                echo("<script>showEmailErrorToast();</script>");
            }else{
                $sql = "UPDATE `admin` SET `email` = '{$new_email}' WHERE `user_id` = '{$admin_user_id}'";
                mysqli_query($conn, $sql) or die("Query failed!");
                mysqli_close($conn);
        ...
        if(isset($_POST["delete"])){
            require_once("DB.php");
            $sql = "DELETE FROM `admin` WHERE user_id = '$admin_user_id'";
            mysqli_query($conn, $sql) or die("Query failed!");
            session_destroy();
